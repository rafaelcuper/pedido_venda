...
uses
  ...
  FireDAC.Comp.DataSet, Vcl.Mask, Vcl.ComCtrls, Vcl.Buttons,
  Datasnap.DBClient, Vcl.Grids, Vcl.DBGrids, Vcl.ExtCtrls, System.IniFiles,

  classeCliente, classeProduto, classePedido;
...
begin
  ...
  try
      ArqIni := TIniFile.Create(ExtractFilePath(Application.ExeName) + '\pedido_venda.ini');
      FDConn.Params.Values['database']  := ArqIni.ReadString('CONEXAO', 'Database', FDConn.Params.Values['database']);
      FDConn.Params.Values['user_name'] := ArqIni.ReadString('CONEXAO', 'Usuario', FDConn.Params.Values['user_name']);
      FDConn.Params.Values['server']  := ArqIni.ReadString('CONEXAO', 'Servidor', FDConn.Params.Values['server']);
      FDConn.Params.Values['password']  := ArqIni.ReadString('CONEXAO', 'Senha', FDConn.Params.Values['password']);
    finally
      ArqIni.Free;
    end;
